...
> **Regra de ouro:** se há conflito entre fontes, o `tokens.css` deste projeto é a verdade. (`colors_and_type.css` está deprecado — back-compat só.) Onde a realidade divergir, `MIGRATION.md` diz como convergir.
...
### ✅ Radius — escala canônica confirmada

Resolvido em 2026-05-29. A fonte de verdade é o `tokens.css` v1.1.0 (idêntico
ao `tokens.css` canônico do projeto Hub — confirmado byte-a-byte). Escala oficial:

| | xs | sm | md | lg | xl | 2xl | 3xl | pill |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Canônico** | 6 | 8 | **10** | 14 | 16 | 18 | 24 | 999 |

A escala "contida" (2/4/6/8/10) que o `MIGRATION.md §1.1` propunha foi
**descartada** — contradizia o canônico. `MIGRATION.md` já foi reescrito pra
instruir o `@simplafy-tec/ui` a adotar a escala canônica acima.

### ✅ Legado `colors_and_type.css` — convertido em shim

Resolvido em 2026-05-29. O `colors_and_type.css` deixou de ter tokens próprios:
agora **importa `tokens.css`** e só adiciona a camada de aliases de produto
(`--color-*`), chart, doctor-palette e utilitários `.ds-*`. **Não pode mais
divergir** do canônico (radius, sombra e fontes são herdados).
