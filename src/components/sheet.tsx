...
const sheetVariants = cva(
  ...
  {
    variants: {
      /* O protótipo não tem seletor de drawer — globals.css:168 e
       * tokens.css §7 nomeiam --radius-lg (10px ≡ --r-xl) como "cards de
       * destaque, drawers" mesmo sem HTML próprio. Só a borda LIVRE (oposta
       * ao lado fixo na tela) recebe raio — o lado colado na viewport fica
       * reto, como qualquer painel deslizante. */
      side: {
...
>(({ side = "right", className, overlayClassName, children, showCloseButton = true, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {showCloseButton && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-primary/20 disabled:pointer-events-none data-[state=open]:bg-secondary">
          <X className="h-4 w-4" />
